...
import edu.carleton.comp4601.assignment2.index.CrawlIndexerServer;
...
public class SDA {
	...
	public Response createDocument(JAXBElement<Document> doc) {
		...
		if(DatabaseManager.getInstance().addNewDocument(document)) {
			...
			CrawlIndexerServer indexer = new CrawlIndexerServer(homePath + luceneIndexFolder, document);
			try {
		...
		try{
			...
			if(existingDocument == null) {
				...
				if(DatabaseManager.getInstance().updateDocument(updatedDocument, existingDocument)) {
					...
					CrawlIndexerServer indexer = new CrawlIndexerServer(homePath + luceneIndexFolder, updatedDocument);
					try {
						indexer.indexHTMLDocument();
						res = Response.ok().build();
					} catch (IOException e) {
